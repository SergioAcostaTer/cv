...
const linkedinProfileSchema = z.object({
  ...
  experience: z
    .array(
      z.object({
        title: z.preprocess((value) => (value == null ? '' : value), z.string()).default(''),
        company: z.preprocess((value) => (value == null ? '' : value), z.string()).default(''),
        employmentType: z.preprocess((value) => (value == null ? '' : value), z.string()).default(''),
        location: z.preprocess((value) => (value == null ? '' : value), z.string()).default(''),
        startDate: z.preprocess((value) => (value == null ? '' : value), z.string()).default(''),
        endDate: z.preprocess((value) => (value == null ? '' : value), z.string()).default(''),
        description: z.preprocess((value) => (value == null ? '' : value), z.string()).default(''),
...
const buildSystemPrompt = (language: string): string => {
  ...
  return [
    'You are an expert technical writer helping software engineers write authentic, professional LinkedIn profiles.',
    'Return only valid JSON. No prose, no markdown, no extra keys.',
    'TONE & STYLE RULES:',
    '- Write in an authentic, conversational, and technically precise first-person tone ("I developed...", "I designed...").',
    '- Ban corporate buzzwords, marketing fluff, and robotic templates.',
    '- DO NOT print literal labels like "Hook:", "Core Expertise:", or "Impact:". Just write the natural text.',
    '- Blend business context (what the system does) with technical execution (how you built it and what tools you used).',
    isSpanish
      ? 'Write all content in Spanish (Spain). Use a natural, professional first-person tone (e.g., "Desarrollo...", "He participado...").'
      : 'Write all content in English. Use a natural, professional first-person tone (e.g., "I develop...", "I contributed to...").',
    'Output schema:',
    '{',
    '  "profile": {',
    '    "fullName": "string",',
    ...
    '      "description": "string 400-1000 chars. 2-3 natural, fluid paragraphs in first-person explaining context, tech, and impact.",',
    '      "techContext": ["string", "..."],',
  ...
  JSON.stringify(
    {
      ...
      hardRules: [
        ...
        'ABOUT SECTION RULES:',
        '- about.descriptionToPaste MUST be 3-4 natural paragraphs.',
        '- Paragraph 1: A natural summary of who you are, your current seniority, and your core focus (e.g. backend, full-stack, distributed systems).',
        '- Paragraph 2: The types of architectures, systems, or domains you have experience building (e.g. payment gateways, high-availability, microservices).',
        '- Paragraph 3: A fluid mention of your preferred tech stack, infrastructure, and engineering practices (Clean Architecture, CI/CD, Agile).',
        '- Paragraph 4: A brief, polite call to action about what you are looking for next.',
        'EXPERIENCE SECTION RULES:',
        '- experience.description MUST be 2-3 natural paragraphs written in the first person.',
        '- Paragraph 1: Describe the overarching business context of the project and your primary role (e.g., "I develop backend systems for...", "I contributed to the modernization of...").',
        '- Paragraph 2: Describe the technical execution. What did you build? How did you integrate technologies? (e.g., "I designed event-driven microservices using Spring Boot and Kafka...").',
        '- Paragraph 3 (if applicable): Mention any major outcomes, migrations, or architectural improvements (e.g., "I participated in migrating legacy PL/SQL jobs to Spring Batch, improving system reliability...").',
        '- NEVER use rigid templates like "Accomplished [X] by doing [Y]". Write fluid, descriptive sentences.',
        'GENERAL RULES:',
        '- Experience achievement bullets must be under 180 characters each and add concrete value.',
        '- Top skills should be a maximum of 15 highly relevant entries (quality over quantity).',
        '- If a detail is missing from the source JSON, do not invent it. Optimize wording around provided info only.',
        'For missing text fields, use empty string ""; for missing lists, use empty arrays [].'
      ]
    },
    null,
    2
...
const buildStrategySystemPrompt = (): string =>
  [
    ...
    'Apply a strict credibility filter: options must match demonstrated years of experience, scope, and domain history.',
    'Do not suggest seniority levels or leadership scope not supported by evidence in the profile data.',
    'When data is ambiguous, choose conservative and realistic progression paths over aspirational leaps.',
    'Output schema:',
    '{',
    '  "analysisSummary": "string",',
    '  "recommended": {',
  ...
  JSON.stringify(
    {
      ...
      rules: [
        ...
        'All selector options must be aligned with demonstrated experience level, actual domain exposure, and proven scope.',
        'Do not propose unrealistic positioning/seniority jumps (example: fintech payments senior tech lead with ~1-2 YOE unless explicitly supported by source data).',
        'Optimize for high-upside but credible pathways: ambitious yet realistic within a 1-2 step progression.',
        'Give the highest scores to immediately credible next steps, and lower scores for stretch paths.',
