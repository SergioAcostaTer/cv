...
const JsonFallback = ({ rawContent }: { rawContent: string }) => (
  <Card>
    ...
    <CardContent>
      <pre className="m-0 overflow-x-auto border border-border bg-muted p-3 text-xs leading-6 text-muted-foreground">
        {rawContent}
      </pre>
    </CardContent>
  </Card>
...
export const MainArea = ({ selectedArtifact, sections, kind, parsedJson, rawContent, onCopySection }: MainAreaProps) => {
  if (!selectedArtifact) {
    return (
      <div className="mx-auto my-24 max-w-xl text-center text-muted-foreground">
        <h2 className="m-0 text-xl font-extrabold text-foreground">Ready</h2>
  ...
  if (kind === 'pdf') {
    return (
      <iframe
        className="h-[62vh] w-full border border-border bg-background lg:h-[calc(100vh-200px)]"
  ...
  return (
    <div className="mx-auto my-24 max-w-xl text-center text-muted-foreground">
      <h2 className="m-0 text-xl font-extrabold text-foreground">Unsupported Artifact</h2>
